...
void loop ()
{
  ...
  byte h[] =   {
    0,3, 1,3, 2,3  };
  cubeFrame* f[] = {
    cube.createFrame(a, 6, 80),
    cube.createFrame(b , 6, 70),
    cube.createFrame(c , 6, 60),
    cube.createFrame(d , 6, 50),
    cube.createFrame(e , 6, 40),
    cube.createFrame(t , 6, 30),
    cube.createFrame(g , 6, 20),
    cube.createFrame(h , 6, 10)
    };
#ifdef DEBUG
    showmem("before free");
#endif
  ...
  byte A[] = {2,5,
              1,2, 1,5, 1,8,
              0,2, 0,8};
  cube.lightSequence(C, sizeof(C), 100);
  cube.lightSequence(E, sizeof(E), 90);
  cube.lightSequence(C, sizeof(C), 100);
  cube.lightSequence(I, sizeof(I), 210);
  cube.lightSequence(L, sizeof(L), 200);
  cube.lightSequence(I, sizeof(I), 210);
  cube.lightSequence(A, sizeof(A), 180);
